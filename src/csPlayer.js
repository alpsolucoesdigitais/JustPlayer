    ...
    YtSetup: (videoTag,playerTagId,defaultId) => {
        ...
        return new Promise((resolve, reject) => {
            csPlayer.csPlayers[videoTag]["videoTag"] = new YT.Player(playerTagId,{
                ...
                playerVars:{
                    controls: 0,
                    mute: 1,                // vídeo começa mutado para autoplay no mobile
                    autoplay: 1,
                    disablekb: 1,
                    color: "white",
                    fs: 0,
                    playsinline: 1,        // importante para mobile iOS
                    rel: 0,
                    loop: 0,
                    cc_load_policy: 3,
                    showinfo: 0,
                ...
                events:{
                    'onReady': (event) => {
                        event.target.mute();     // garante o mute na primeira reprodução
                        event.target.playVideo(); // toca automaticamente
        ...
        function updateTextTime(){
            ...
            parent.querySelector(".csPlayer-controls-box .csPlayer-controls p:nth-of-type(1)").innerHTML = formatTime(currentTime);
            parent.querySelector(".csPlayer-controls-box .csPlayer-controls p:nth-of-type(2)").innerHTML = formatTime(duration);
        }
